package com.kostya.scalesnetwork.provider;
...
public class TaskTable {
    ...
    public enum TaskType {
        /** событие для електронной почты. */
        TYPE_EVENT_SEND_MAIL,
        /** событие для облака. */
        TYPE_EVENT_SEND_HTTP_POST,
        /** чек для google disk. */
        TYPE_EVENT_SEND_SHEET_DISK,
        /** чек для смс отправки контакту. */
        TYPE_EVENT_SEND_SMS_CONTACT,
    ...
    public Uri insertNewTaskEmail(TaskType mimeType, long documentId, long dataId, String address) throws Exception {
        ContentValues newTaskValues = new ContentValues();
        newTaskValues.put(KEY_MIME_TYPE, mimeType.ordinal());
        newTaskValues.put(KEY_DOC, documentId);
        newTaskValues.put(KEY_ID_DATA, dataId);
        newTaskValues.put(KEY_DATA0, address);
        newTaskValues.put(KEY_DATA1, new SystemTable(mContext).getProperty(SystemTable.Name.USER_GOOGLE));
        newTaskValues.put(KEY_DATA2, new SystemTable(mContext).getProperty(SystemTable.Name.PASSWORD));
        newTaskValues.put(KEY_NUM_ERROR, 0);
        return mContext.getContentResolver().insert(CONTENT_URI, newTaskValues);
    }
